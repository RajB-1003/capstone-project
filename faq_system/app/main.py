...
from config.settings import THRESHOLD_HIGH, THRESHOLD_LOW
...
def load_pipeline_fixtures():
    from app.storage.db import get_all_faqs
    from app.core.embedder        import load_embedding_model, load_and_embed_faqs
    from app.core.keyword_search  import build_bm25_index
    from app.services.router    import load_regex_patterns
    from app.services.router_tier2    import load_intent_exemplars, embed_intents
    from app.storage.embedding_store import load_embeddings, save_embeddings

    PATTERNS_PATH  = os.path.join("data", "configs", "regex_patterns.json")
    EXEMPLARS_PATH = os.path.join("data", "configs", "intent_exemplars.json")
...
def render_navbar(active_response=None, cur_user=None):
    from app.storage.cache       import cache_stats, clear_all_caches
    from app.storage.query_store import get_categories, get_all_tags
        ...
        for idx, (page_id, label) in enumerate(nav_items):
            ...
            with btn_cols[idx]:
                if st.button(
                    ...
                    if _token:
                        from app.storage.db import update_token_last_page
...
from app.ui.comparison    import compare_retrieval
from app.storage.query_store   import load_faqs, add_faq, edit_faq, delete_faq, get_categories
from app.services.rag_service      import generate_rag_answer
from app.core.multilingual  import process_query as multilingual_process_query
...
if active_page == "search":
    ...
    with col:
        ...
        with voice_col:
            from app.utils.helpers import get_voice_input, normalize_voice_query
    ...
    if search_clicked and query.strip():
        ...
        if _user_id:
            from app.storage.db import store_query
            store_query(
                _user_id, 
                query.strip(), 
