...
def get_poor_faqs_from_db() -> list[dict]:
    ...
    """
    try:
        from app.storage.feedback_store import get_aggregated_scores, POOR_SCORE_THRESHOLD
...
def render_admin_dashboard() -> None:
    ...
    try:
        import pathlib
        from app.storage.feedback_store import FEEDBACK_LOG_PATH
    ...
    try:
        from app.storage.feedback_store import get_aggregated_scores
        agg = get_aggregated_scores()
    except Exception:
        agg = {}
            ...
            with st.container(border=True):
                ...
                with c2:
                    ...
                    if st.button("🗑️ Ignore", key=btn_key_rem, use_container_width=True):
                        from app.storage.db import delete_unanswered_query
                    ...
                    with st.form(key=f"form_{hash(q)}"):
                        ...
                        if st.form_submit_button("Submit FAQ"):
                            if not new_ans.strip():
                                ...
                                from app.storage.db import add_faq, delete_unanswered_query
                                from app.storage.embedding_store import delete_embeddings
